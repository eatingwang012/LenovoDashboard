...
import {
  CONSUMER_COLOR_PALETTE as COLORS,
  CONSUMER_REPLACEMENT_DRIVER_DEFINITIONS as replacementDriversDefinitions,
} from "../mock/consumerInsights";
import { useConsumerInsightsData } from "../hooks/useConsumerInsightsData";
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  const title = label ?? payload[0]?.payload?.stage ?? payload[0]?.payload?.factor ?? payload[0]?.name ?? "Detail";
    ...
    <div className="bg-white p-3 border border-slate-200 shadow-xl rounded-lg text-xs">
      ...
      <div className="space-y-1">
        {payload.map((item: any, index: number) => {
          ...
          const shouldAppendPercent =
            (typeof itemName === "string" && itemName.includes("%")) ||
            item?.dataKey === "percentage" ||
            item?.dataKey === "sentiment";
          const numericValue = typeof item?.value === "number" ? item.value.toLocaleString() : item?.value ?? "-";

          return (
            <div key={index} className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: item?.fill || item?.color || "#94a3b8" }} />
              <span className="text-slate-500 whitespace-nowrap">{itemName}:</span>
              <span className="font-black text-slate-900 ml-auto">
                {numericValue}{shouldAppendPercent ? '%' : ''}
              </span>
            </div>
          );
        })}
                        ...
                        <div className="flex-1 pb-4">
                          ...
                          {index > 0 && (
                            <p className="text-[9px] text-red-500 font-medium mt-1">
                              â†?{((1 - stage.value / derivedData.decisionFunnel[index-1].value) * 100).toFixed(1)}% Drop-off
                            </p>
                          )}
                        </div>
                      </div>
