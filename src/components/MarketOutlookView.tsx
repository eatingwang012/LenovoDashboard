...
import { useMarketDynamicsData } from "../hooks/useMarketDynamicsData";
...
const ICON_MAP = { Cpu, Monitor, HardDrive, BatteryCharging, Microchip };
...
export function MarketOutlookView() {
  ...
  const renderTimelineNodes = (events: any[], trackType: "product" | "supply") => {
      ...
      <div className="space-y-4 pt-8 border-t border-slate-200">
         ...
         <div className="grid md:grid-cols-5 gap-4">
            {supplyCategories.map(cat => {
               const Icon = ICON_MAP[cat.icon as keyof typeof ICON_MAP] ?? Cpu;
         ...
         <Card className="border-none shadow-sm ring-1 ring-slate-200 mt-6 bg-slate-900 text-slate-100 overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-slate-800 bg-slate-950/50">
               <div>
                 ...
                 <CardDescription className="text-xs font-serif italic text-slate-400 mt-1">2025 â€?2026E Quarterly Index (Base 100 at 25Q1)</CardDescription>
               </div>
